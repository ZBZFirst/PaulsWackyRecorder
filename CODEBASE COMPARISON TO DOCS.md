...
- New/Open/Archive/Restore table
- Add/Delete columns
- Delete selected row
- Export CSV
...
## 6) Documentation quality and link integrity gaps

### Observed issues in docs
- `TableManagementScreen.md` references `[[MAP_MainMenu]]`, `[[MAP_TopNavigation]]`, and `[[MAP_Screen*]]` style links that are not standard Markdown and appear unresolved in this repository.
- “Planned Buildout” language indicates the page is partially aspirational rather than strictly current-state.
- Related document section has formatting/clarity issues (leading backtick + sentence fragment style).
...
1. Convert placeholder `[[MAP_*]]` tokens in `docs/TableManagementScreen.md` to concrete relative markdown links (or remove if pages do not exist).
2. Add a “Current Implementation vs Planned Integration” section to `docs/PaulsUSDatasetExplained.md` clarifying that workbook-sheet import is not yet fully wired.
3. If workbook integration is intended next, add a short contract doc specifying:
   - parsing boundary,
