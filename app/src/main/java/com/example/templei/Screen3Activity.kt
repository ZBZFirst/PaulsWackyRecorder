...
class Screen3Activity : ComponentActivity() {
    ...
    private fun scanFolderClips(folder: DocumentFile, folderName: String): List<ClipMetadata> {
        return folder.listFiles().filter { it.isFile }.mapNotNull { file ->
            ...
            val durationMs = readDurationMs(file.uri) ?: return@mapNotNull null
            if (durationMs !in 1..MAX_SOUND_DURATION_MS) return@mapNotNull null
            ClipMetadata(file.uri.toString(), fileName, file.uri, folderName, durationMs)
    ...
    private fun renderClipBrowser(clips: List<ClipMetadata>) {
        ...
        clips.forEach { clip ->
            val button = Button(this).apply {
                ...
                setOnClickListener { attemptPlayback(clip) }
                setOnLongClickListener {
                    showAssignClipDialog(clip)
                    true
                }
            }
            clipBrowserContainer.addView(button)
        }
    }
    ...
    private fun refreshReadyState() {
        ...
        stateMachine.onReady(
            ...
            playableCount = activeFolderClips.size,
            activeStreams = activeStreamIds.size,
    ...
    private data class ClipMetadata(val id: String, val displayName: String, val uri: Uri, val folderName: String, val durationMs: Long)
